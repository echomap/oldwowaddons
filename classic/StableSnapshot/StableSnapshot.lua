...
local options = {
    ...
    args = {
        msg = {
            type = 'input',
            name = 'StableSnapshot',
            desc = 'Classic version of StableSnapshot',
            get  = 'GetMyStable',
        },
    },
}
...
function StableSnapshot:SlashFunc(input)
    ...
    local debugMeStr = string.format("(debug is %s )", tostring(debugMe) )
		print( L["Addon_Desc"] )
		print("/strack <gui/debug/hide/version> " .. debugMeStr )
		print("/strack <showtest/guitest> " .. debugMeStr )
    --print("/strack <CountByFamily/CountByFamilyGui/CountBySpecial/CountBySpecialGui> " .. debugMeStr )
	end   
	if input == 'show' then
